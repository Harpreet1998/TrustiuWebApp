...
namespace TrustiuWebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> ProductBySubCategory(int? id)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SellProduct([Bind("ProductID,ProductName,FileUpload,Description,Price,SubCategoryID")] Product product)
        {
            ModelState.Remove("UserName");
        ...
        {
            string userid = _userManager.GetUserName(this.User);
            var applicationDbContext = _context.Products.Include(p => p.SubCategory).Where(p => p.UserName == userid);
            return View(await applicationDbContext.OrderBy(r => Guid.NewGuid()).ToListAsync());
        }
